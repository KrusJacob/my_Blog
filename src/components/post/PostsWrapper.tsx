  ...
  return (
    <div className="mt-2  bg-opacity-70  rounded-3xl   ">
      <div className="flex text-3xl md:text-5xl justify-between items-center   rounded-tl-3xl  rounded-tr-3xl bg-[var(--purpleColor)]">
        ...
        >
          <CiSquarePlus />
          <span className="text-2xl text-[var(--purpleColor)] mr-2 px-2 md:block hidden">
            {t("posts.newPost")}
          </span>
        </button>
        <div>
          <h2 className="text-center text-3xl text-white">{t("posts.title")}</h2>
        </div>
        <button
          onClick={() => refetch()}
        ...
        >
          <span className="text-2xl text-[var(--purpleColor)] mr-2 px-2 md:block hidden">{t("posts.update")}</span>
          <IoReload />
        </button>
      </div>
      {children}
